    ...
    public class CustomWizardCommandHandler : ICommandHandler {
        ...
        void OnNewReportWizard() {
            ...
            mdiController.Form.BeginInvoke(new MethodInvoker(() => {
                ...
                System.Action myMethod = delegate() { new XtraWizardReportBuilderWrapper(designerHost).Build(runner.WizardModel, true, CreateDataComponent(runner.WizardModel)); };
                undoEngine.ExecuteWithoutUndo(myMethod);
            }));
        }
        public void MyMethod(IDesignerHost designerHost, CustomReportWizardRunner runner) {
            new XtraWizardReportBuilderWrapper(designerHost).Build(runner.WizardModel, true, CreateDataComponent(runner.WizardModel));
        }
